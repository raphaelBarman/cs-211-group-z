...
void draw()
{
  ...
  for (int[] quad : quads) {
    ...
    if (qg.isConvex(c12, c23, c34, c41) && qg.validArea(c12, c23, c34, c41, base_img.width*base_img.height, 5000) && qg.nonFlatQuad(c12, c23, c34, c41)) {
      ...
      ;
      PVector p1 = PVector.sub(c12, ln);
      PVector p2 = PVector.add(c23, ln);
      line(p1.x, p1.y, p2.x, p2.y);
      ln = PVector.sub(c23, c34).mult(far);
      ;
      p1 = PVector.sub(c23, ln);
      p2 = PVector.add(c34, ln);
      line(p1.x, p1.y, p2.x, p2.y);
      ln = PVector.sub(c34, c41).mult(far);
      ;
      p1 = PVector.sub(c34, ln);
      p2 = PVector.add(c41, ln);
      line(p1.x, p1.y, p2.x, p2.y);
      ln = PVector.sub(c41, c12).mult(far);
      ;
